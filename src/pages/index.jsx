...
class MovableBlock extends Component {
  render() {
    ...
    return (
      <div>
        <p className="text-1xl sm:my-2 italic">
          and this is the place where my ideas grow
        </p>
        <div class="flex justify-center pt-2">
          <div class="flex sm:mt-4 space-x-6">
  ...
  if (focus === false) {
    return (
      <div className="mt-8 mx-3">
        ...
        <h1 className="text-xl mt-2 mb-2">Recently updated notes:</h1>
      </div >
    )
  } else {
    return (
      <h1 className="text-xl my-4">Search results:</h1>


    )
  }
}
const Index = ({ posts, recentPosts }) => {


  const [focus, setFocus] = React.useState(false);
  const [currentPosts, setCurrentPosts] = React.useState(recentPosts);
  const onFocus = () => {
    setFocus(true)
    setCurrentPosts(posts);
  }
  const onBlur = () => {
    // sleep for 100ms to wait for the search to finish
    setTimeout(() => {
      setFocus(false)
  ...
  return (
    <main className="flex flex-col">
      ...
      <TopNav onFocus={onFocus} onBlur={onBlur} className="inline-block" />
      <About focus={focus} />
...
export const getStaticProps = async () => {
  ...
  let posts = allPosts
    ...
    // sort by date with moment
    .sort((a, b) => moment(b.mtime, 'YYYYMMDDHHmmss').diff(moment(a.mtime, 'YYYYMMDDHHmmss')))
